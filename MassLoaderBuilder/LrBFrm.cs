    ...
    public partial class LrBFrm : Form
    {
        protected void Controls_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button.Equals(MouseButtons.Left))
            {
                ...
                var m = Message.Create(this.Handle, 0xa1, new Hwnd(0x2), Hwnd.Zero);
                WndProc(ref m);
            }
        }
        public LrBFrm()
        ...
        private void CleanLink_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ControlActive.CheckMessage(this.StatusMessage, "Выберите элемент из списка для удаления.", Color.YellowGreen, 5000);
            }
        }
        private void CleanAll_Click(object sender, EventArgs e)
        {
            ...
            if (this.LinkBox.Items.Count != 0 || !string.IsNullOrWhiteSpace(this.LinkTextBox.Text))
            {
                ...
                ControlActive.CheckMessage(this.StatusMessage, "Все поля отчищены!", Color.YellowGreen, 5000);
            }
            else
            {
                this.StatusMessage.Location = new Point(150, 334);
                ControlActive.CheckMessage(this.StatusMessage, "Нечего отчищать", Color.YellowGreen, 5000);
            }
        }
        ...
        private async void Build_Click(object sender, EventArgs e)
        {
            ...
            if (CheckIsEmptyBoxs())
            {
                ...
                ControlActive.CheckMessage(this.StatusMessage, "Заполните все поля!", Color.YellowGreen, 5000);
            }
            else
            {
                ...
                {
                    this.StatusMessage.Location = new Point(515, 371);
                    ControlActive.CheckMessage(this.StatusMessage, "Выберите путь сохранения!", Color.YellowGreen, 5000);
                }
                else
                {
                    ...
                    {
                        this.StatusMessage.Location = new Point(515, 371);
                        ControlActive.CheckMessage(this.StatusMessage, "Нет ссылок для загрузки!", Color.YellowGreen, 5000);
                    }
                    else
                    {
        ...
        {
#if !DEBUG
            MemoryPE.Initialize(); // Вызываем защиту от AntiDump'а
#endif
        ...
        private void AddLink_Click(object sender, EventArgs e)
        {
                ...
                if (rgx.IsMatch(this.LinkTextBox.Text))
                {
                    if (this.LinkTextBox.Text.EndsWith(".exe", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        else
                        {
                            ...
                            ControlActive.CheckMessage(this.StatusMessage, "Такая ссылка уже есть в списке!", Color.YellowGreen, 5000);
                        }
                    }
                    else
                    {
                        this.StatusMessage.Location = new Point(505, 371);
                        ControlActive.CheckMessage(this.StatusMessage, "Расширения файла только .exe", Color.YellowGreen, 5000);
                    }
                }
                else
                {
                    this.StatusMessage.Location = new Point(515, 371);
                    ControlActive.CheckMessage(this.StatusMessage, "Некорректная ссылка", Color.YellowGreen, 5000);
                    this.LinkTextBox.Clear();
                }
            }
            else
            {
                this.StatusMessage.Location = new Point(505, 371);
                ControlActive.CheckMessage(this.StatusMessage, "Введите ссылку в поле ввода", Color.YellowGreen, 5000);
            }
        }
        ...
        private void DownloadLinkBox_Click(object sender, EventArgs e)
        {
            ...
            {
                this.StatusMessage.Location = new Point(515, 371);
                ControlActive.CheckMessage(this.StatusMessage, "Проверка загрузки..", Color.YellowGreen, 5000);
        ...
        private async void CheckLinkBox_Click(object sender, EventArgs e)
        {
            if (this.LinkBox.SelectedIndex != -1)
            {
                ...
                {
                    this.StatusMessage.Location = new Point(515, 371);
                    ControlActive.CheckMessage(this.StatusMessage, "Проверяем...", Color.YellowGreen, 5000);
                    ...
                    else
                    {
                        ...
                        {
                            this.StatusMessage.Location = new Point(505, 371);
                            ControlActive.CheckMessage(this.StatusMessage, "Ссылка с файлом рабочая!", Color.YellowGreen, 4000);
                        }
                        else
                        {
                            this.StatusMessage.Location = new Point(505, 371);
                            ControlActive.CheckMessage(this.StatusMessage, "Ссылка с файлом не рабочая!", Color.YellowGreen, 4000);
                        }
                    }
        ...
        private void LinkBox_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            foreach (int selectedIndex in this.LinkBox.SelectedIndices)
            {
                ...
                {
                    e.DrawBackground();
                    using Brush brush = new SolidBrush(Color.FromArgb(30, 149, 80));
